...
import { objectOptionsJSON } from '@_interface/JSONData';
...
@Component({
  tag: 'three-canvas-color',
  shadow: false,
})
export class ThreeCanvasColor {
  ...
  /** set of colors to choose from in JSON format */
  @Prop({ mutable: false }) colors: string = '';
  ...
  private _optionColors: objectOptionsJSON;
  ...
  componentWillLoad() {
    ...
    this._optionColors = JSON.parse(this.colors);
  ...
  private _pointerActiveCall = (x: number, y: number, hand: string): void => {
    this._buttonsActive(x, y, hand, this._optionColors);
  }

  private _pointerPassiveCall = (x: number, y: number): void => {
    this._buttonsPassive(x, y, this._optionColors);
  }
  ...
  private _addToPoolDynamic(): void {
    ...
    // Add panel to pool - attached to left controller
  ...
  private _uiEvents(eventName: string, value: string, from: string, uuid: string): void {
  ...
  private _buttonsActive(x: number, y: number, hand: string, options: objectOptionsJSON): void {
    let colors: string[] = options.colors;
    for (let i = 0; i < colors.length; i++) {
      uiButtonRound(
        this._canvasContext,                            // ctx
        x, y, this._intersectData[hand].state,          // mouseX, mouseY, mouseState
        ((0.12 + (0.12 * i)) * this._resMultiplier),    // x
        (0.12 * this._resMultiplier),                   // y
        (0.05 * this._resMultiplier),                   // radius
        colors[i],                                      // color
        () => { this._uiEvents(this._intersectData[hand].state, colors[i], options.from, this._originObjectID) }      // callback
      );
    }
  }
